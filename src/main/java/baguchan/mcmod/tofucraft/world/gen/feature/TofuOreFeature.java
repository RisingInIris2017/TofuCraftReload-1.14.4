...
public class TofuOreFeature extends Feature<TofuOreConfig> {
    public TofuOreFeature(Function<Dynamic<?>, ? extends TofuOreConfig> deserialize) {
        super(deserialize);
    }

    @Override
    public boolean place(IWorld world, ChunkGenerator<? extends GenerationSettings> generator, Random random, BlockPos pos, TofuOreConfig config) {
                            ...
                            for (int bz = minBZ; bz <= maxBZ; ++bz) {
                                ...
                                if (deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ < 1.0) {
                                    ...
                                    if (!mask.get(maskIndex)) {
                                        mask.set(maskIndex);
                                        pos.setPos(bx, by, bz);
                                        if (world.getBlockState(pos).getBlock() == TofuBlocks.TOFUTERRAIN) {

...
public class TofuNetwork {
    ...
    /*
     * Feature:
     * 1. Maintains a hashmap of all TofuEnergyStorageTileEntity(TESTE)
    ...
    public static void onUnloadWorld(WorldEvent.Unload event) {
        ...
        for (String uid : toUUIDs(Instance.getTEWithinDim(world.getDimension().getType().getId()))) {
    ...
    public Stream<Map.Entry<String, TileEntity>> getTEWithinDim(int dimid) {
        return reference
                .entrySet()
                .stream()
                .filter(entry -> entry.getValue().getWorld().getDimension().getType().getId() == dimid);
    }

    public Stream<Map.Entry<String, TileEntity>> getTEWithinRadius(World world, BlockPos pos, double radius) {
        return getTEWithinDim(world.getDimension().getType().getId())
                .filter(entry -> entry.getValue().getDistanceSq(pos.getX(), pos.getY(), pos.getZ()) <= radius);
    }

    public Stream<Map.Entry<String, TileEntity>> getTEWithinRadius(TileEntity center, double radius) {
        BlockPos pos = center.getPos();
        return getTEWithinDim(center.getWorld().getDimension().getType().getId())
                .filter(entry -> entry.getValue().getDistanceSq(pos.getX(), pos.getY(), pos.getZ()) <= radius);
    }
